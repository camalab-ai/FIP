...
import glob
import logging
from random import choices, randint # requires Python >= 3.6
import numpy as np
import cv2
import torch
from skimage.metrics import peak_signal_noise_ratio

IMAGETYPES = ('*.tif', '*.tiff') # Supported image types
...
def normalize_augment(datain):
	...
	def transform(sample):
		...
		w_aug = [24, 24, 24, 24] # one fourth chances to do_nothing
		transf = choices(aug_list, w_aug)

		# transform all images in array
		return transf[0](sample[0])[:, :, :-1, :-1], transf[0](sample[1])[:, :, :-1, :-1]


	#augment
	img_train, gt_train = transform(datain)

	# extract ground truth (central frame)
	img_train = (img_train - 240) / (2 ** 12 - 1 - 240)
	gt_train = (gt_train - 240) / (2 ** 12 - 1 - 240)
...
def open_raw_sequence(seq_dir, gray_mode, expand_if_needed=False, max_num_fr=100, outdoor=False, file_num=0):
	r""" Opens a sequence of images and expands it to even sizes if necesary
	Args:
		fpath: string, path to image sequence
		gray_mode: boolean, True indicating if images is to be open are in grayscale mode
	...
	if not outdoor:
		if file_num is None:
			names = ["frame" + str(i+1) +"_noisy" + str(randint(0, 9)) + ".tiff" for i in range(7)]
			files = [file for file in files if any([name in file for name in names])]
		else:
			files = [file for file in files if '_'+str(file_num)+'.tiff' in file]
	seq_list = []
	print("\tOpen sequence in folder: ", seq_dir)
	for fpath in files[0:max_num_fr]:
...
def open_image(fpath, gray_mode, expand_if_needed=False, expand_axis0=True, normalize_data=True):
	...
	if not gray_mode:
		...
		img = (cv2.cvtColor(img, cv2.COLOR_BGR2RGB)).transpose(2, 0, 1).astype(np.float32)
	else:
...
def batch_psnr(img, imclean, data_range, val=True):
	r"""
	...
	if not val:
		for i in range(img_cpu.shape[0]):
			psnrs.append(peak_signal_noise_ratio(imgclean[i], img_cpu[i], data_range=data_range))
	else:
		for i in range(3, img_cpu.shape[0]-3):
			psnrs.append(peak_signal_noise_ratio(imgclean[i], img_cpu[i], data_range=data_range))
